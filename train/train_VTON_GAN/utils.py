...
def save_checkpoint(model, optimizer_G, optimizer_D, epoch, global_step, loss, output_dir=None, filename=None):
    ...
    if hasattr(config, 'S3_BUCKET_NAME') and config.S3_BUCKET_NAME:
        s3_prefix = getattr(config, 'CHECKPOINT_S3_PREFIX', 'checkpoints/vtongan')
...
def load_checkpoint(checkpoint_path, model, optimizer_G=None, optimizer_D=None):
    ...
    try:
        ...
        # Load model state
        if 'generator' in checkpoint['model_state_dict']:
            model.generator.load_state_dict(checkpoint['model_state_dict']['generator'])
...
def load_latest_checkpoint(model, optimizer_G=None, optimizer_D=None, output_dir=None):
    ...
    if os.path.exists(latest_path):
        ...
        ckpt = load_checkpoint(latest_path, model, optimizer_G, optimizer_D)
        if ckpt:
    ...
    if hasattr(config, 'S3_BUCKET_NAME') and config.S3_BUCKET_NAME:
        ...
        if downloaded_path:
            ckpt = load_checkpoint(downloaded_path, model, optimizer_G, optimizer_D)
            if ckpt:
                return ckpt.get('epoch', 0), ckpt.get('global_step', 0)
