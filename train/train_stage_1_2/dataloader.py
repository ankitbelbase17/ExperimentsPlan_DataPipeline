...
def get_dataloader(tokenizer, batch_size=None):
    ...
    if len(ds1) == 0 and len(ds2) == 0:
        ...
        return DataLoader(ds1, batch_size=batch_size) # return empty loader
        
    combined_dataset = ConcatDataset([ds1, ds2])
    
    # Calculate Weights for WeightedRandomSampler
    # Target: 30% from DS1, 70% from DS2
    # weight = target_prob / number_of_samples
    
    n1 = len(ds1)
    n2 = len(ds2)
    
    # Default to uniform if one is empty to avoid div by zero
    w1 = config.DATASET_WEIGHTS[0] / n1 if n1 > 0 else 0
    w2 = config.DATASET_WEIGHTS[1] / n2 if n2 > 0 else 0
    
    # weights array corresponding to each sample in the combined dataset
    samples_weights = [w1] * n1 + [w2] * n2
    
    # Check if total weights > 0
